...
/**
 * Mostra um arquivo .py com realce de sintaxe e numeração de linhas.
 * Linhas anotadas recebem um marcador; a linha em foco fica destacada e sua
 * nota aparece logo abaixo do código.
 */
export function CodeBlock({ arquivo, anotacoes, linhaFoco }: Props) {
  const linhas = useMemo(() => {
    ...
    const realcado = highlight(codigo, { language: 'python', ignoreIllegals: true });
    ...
    <Box flexDirection="column">
      <Box flexDirection="column" borderStyle="round" borderColor="gray" paddingX={1}>
        {linhas.map((linha, i) => {
          ...
          return (
            <Box key={n}>
              <Text color={foco ? 'yellowBright' : temNota ? 'yellow' : 'gray'}>
                {marcador}{' '}
              </Text>
